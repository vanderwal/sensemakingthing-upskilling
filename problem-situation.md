# Multiple Sources of Conflicting Information Lead to Voter Inaction

> A problem situation mapped using Kees Dorst's Frame Creation method.
> Built from 6 cards covering 8 signals across the evidence ladder.
> Exported 2026-07-25T20:46:06.392Z.

## The situation
Providing an easy means to use a dependable and a reliable resource of evidence-based understanding supports the comfort and confidence in civic participation and voting.

### Why this is open, not bounded
The broad set of information resources and conflicting information broadcast and shared from convenient resources can compound efforts to have alternate evidence-based information used.

### Evidence trail
• Theme: Evidence-based Service for Voting  — Voting and civic engagement, even getting to the polling station to vote need a means to find valid and correct information that is easy to find and validate.
  • Pattern: Evidence based Information need — Evidence-based information fills a gap in understanding 
    • Evidence: Evidence based information needed — Trusted evidence-based information sources are neededi
      - Signal "Civic Distrust & Disinformation": A toxic combination of distrust in voting, a lack of civics literacy, and internet-era disinformation is allowing political entities to manipulate voters' worst instincts.
      - Signal "Media Misinformation": Broad concerns regarding how misleading media and digital influencing are negatively impacting civic engagement.
    • Evidence: Evidence-based Information need — The lack of evidence from trustworthy information resources is needed
      - Signal "Media Misinformation": Broad concerns regarding how misleading media and digital influencing are negatively impacting civic engagement.
      - Signal "Accountability for Public Lies": Telling the truth — and holding liars accountable and responsible — is named as the missing piece of civic life.
      - Signal "Local Candidate Information Gaps": Voters lack easily accessible, actionable information about local county-level candidates, often relying heavily on name recognition from campaign signs.
  • Pattern: Building Civic Engagement Verified Information — Civic engagement improves working with the same validated information
    • Evidence: Evidence based information needed — Trusted evidence-based information sources are neededi
      - Signal "Civic Distrust & Disinformation": A toxic combination of distrust in voting, a lack of civics literacy, and internet-era disinformation is allowing political entities to manipulate voters' worst instincts.
      - Signal "Media Misinformation": Broad concerns regarding how misleading media and digital influencing are negatively impacting civic engagement.
    • Evidence: Civic Engagement — Civic engagement is low and people aren't seeing through line connections with their government and representative
      - Signal "Broad Civic Disengagement": Many people are not engaged in local or national politics, don't know who the candidates are or what they stand for, and don't vote — despite candidate forums and get-out-the-vote efforts.
      - Signal "Anemic, Uneven Participation": Civic participation is thin, and only some voices get heard and lifted.
      - Signal "Misunderstood Pace of Government": Constituents misjudge who holds which powers and how fast government was designed to move, so legitimate slowness reads as failure — and stretched congressional staff can't give every issue the weight visitors expect.
      - Signal "Media Misinformation": Broad concerns regarding how misleading media and digital influencing are negatively impacting civic engagement.
      - Signal "Civics Talent Pipeline Shortage": There is a noticeable shortage of talent to fill junior roles within the civics and elections workforce.

---

## Evidence landscape
### What the evidence covers
Strong on misinformation and distrust of resources and institutions that stems from it. It is weak on drivers to have people engage and check evidence-based resources. Not a lot of lessons learned from existing resources or prior efforts.

### Gaps — what I think is still thin
Gap: Why people haven't used evidence-based resources in the past for voting information and polling facts

Question: What are lessons learned from prior fact checking easy to use resources and the use and reliability that people felt these resources had. I have a hunch that having a quick and easy resource to find out about targeted civics / voting / polling. 

Method: Find research on methods and outcomes from evidence-based resources and effectiveness. Find research on effectiveness on tightly focused resources versus broader offerings.

---

## Research access
### Who I need to hear from
Need to check with people like Dana Chisnell to find sources for research on election / polling fact checking research. Connect with local government election information as an evidence source. Academic research projects and focus research.

### What I can't reach alone — and how I might
Need: Election polling facts and changes 
How to get: Election Boards

---

## The paradox
The effort to create a voting / polling evidence-based resource that is easy to use may not be used for the unknown reasons people don't check currently.

_Sharpness self-check (pod-declared): 0/3._

### Who benefits from the status quo
Actor: Those not wanting large voter turn out
Mechanism: Seeding misinformation and fear

### How we know (evidence as warrant)
No client handed us this problem — the evidence did. This paradox was read off **8 source extracts** across **1 named theme** (Evidence-based Service for Voting). The full trail ships with this folder (`data/extracts.csv` + the themes narrative).

Representative extracts:
- "Civic Distrust & Disinformation" — A toxic combination of distrust in voting, a lack of civics literacy, and internet-era disinformation is allowing political entities to manipulate voters' worst instincts.
- "Media Misinformation" — Broad concerns regarding how misleading media and digital influencing are negatively impacting civic engagement.

### What we're still testing
- What could break this framing: Evidence-based resources that are easy to access and use may still not be enough to overcome apathy and those how want people to not participate and vote and spread misinformation.
- Sharpness still unclaimed: it names a contradiction where the fix undoes itself — x requires not-x
- Sharpness still unclaimed: it names who is invested in it persisting
- Sharpness still unclaimed: both legs of the contradiction are evidenced (linked cards / extracts)

### Pressure-testing the framing
Evidence-based resources that are easy to access and use may still not be enough to overcome apathy and those how want people to not participate and vote and spread misinformation.

---

## The field — candidate problem owners
(field not yet mapped — start with the inner circle, then widen, and mark who could own this)

---

## What comes next
This situation is mapped but not solved — and solving it is not the next step. The next steps in Frame Creation are:

1. **Map the field** — identify every player connected to this situation or a future resolution. Define each by their practices and currency, not their title. Look especially for players who share the values your themes surfaced — even in distant, unexpected domains.
2. **Identify a client** — from the potential problem owners, who could own this situation or adopt a resolution? What would they need to see?
3. **Choose a delivery context** — through what kind of vehicle does a resolution reach the field? (An internal team, a government program, an open-source project, a startup, a nonprofit, a civic institution, a co-op — there is no right answer, only trade-offs.)
4. **Deepen the archaeology** — the gaps above name what's still thin. Go find it: interview people, run deep-research sweeps, pull secondary sources.
5. **Create frames** — only after the field is mapped and the archaeology is deep. "If this situation is approached as if it is ___, then ___."

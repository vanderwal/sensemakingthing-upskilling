...
They are working on one Evidence in their web map.

This is an unclassified evidence card. Help the user classify it into ONE of the seven Frame Creation evidence types, and name it. Orient first by analytical movement: ARCHAEOLOGY & CONTEXT — excavating the known problem and its as-is situation (History, Counterfactual, Problem, Boundary, Flux); versus FIELD MAPPING — widening to the players in the broader field and the universal values they share (Player, Value). The seven types: History (What happened / what was done); Counterfactual (The path not taken); Problem (A bounded difficulty, solvable conventionally); Boundary (A "they will never…" nonnegotiable); Flux (A point of movement or potential change); Player (A stakeholder, by practices & currency); Value (A deeper or universal value from the field). Read the constituent signals, decide the movement and then the type that best fits what they triangulate toward, and explain why. If two types are plausible, name the trade-off. Do not decide for the user — help them decide.
</context>

<cluster>
  <tier>Evidence</tier>
  <current_title>Civic Engagement</current_title>
  <current_description>Civic engagement is low and people aren't seeing through line connections with their government and representative</current_description>
  <contents>
...
<task>Help the user find the THROUGH-LINE of this Evidence — the deep mechanism or shared condition that genuinely connects everything underneath, beyond any surface topic. A through-line is not a category label; it is the underlying mechanism, power structure, missing affordance, or human experience that the constituent elements triangulate toward.</task>
